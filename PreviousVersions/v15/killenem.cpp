...
bool checkBounds(SDL_Rect stretch, SDL_Rect fullScreen, int &xpos, int &ypos, int prevxpos, int prevypos, int &num, bool link, bool balive){
                // Check if in bounds
                switch(num){
                    case 0:
                    ...
                    case 4:
                        ...
                        if(balive){
                            if(ypos < 2*fullScreen.h/11 + fullScreen.y){
                             ...
                             if(ypos < 3*fullScreen.h/22 + fullScreen.y - 10 && link){
                                num = 5;
                                xpos = (fullScreen.w - stretch.w) / 2;
                                ypos = 9*fullScreen.h/11 + fullScreen.y - stretch.h;
                            }
...
int main( int argc, char* args[] ){
    ...
    if( !init() ){
        ...
        if( !loadMedia() ){
            ...
            // Variables to check if a character is alive
            bool lalive = true; // link
            bool e0alive = true; // enemy 0
            bool e1alive = true; // enemy 1
            bool e2alive = true; // enemy 2
            bool balive = false; // boss (initially false b/c we dont want to render him until the end
            ...
            while( !quit ){
                ...
                while(SDL_PollEvent( &e ) != 0){
                    ...
                    if( e.type == SDL_QUIT ){
                        ...
                        if(e.key.keysym.scancode == SDL_SCANCODE_A){
                            if(frame == 0 || frame == 1){ // attack down
                                prev = frame;
                                frame = 8;
                                if(stretch.y + stretch.h + 10 > e0ypos && stretch.y + stretch.h + 10 < e0ypos + estretch0.h && stretch.x < e0xpos + estretch0.w && stretch.x + stretch.w > e0xpos){
                                    e0alive = false;
                                }
                                if(stretch.y + stretch.h + 10 > e1ypos && stretch.y + stretch.h + 10 < e1ypos + estretch1.h && stretch.x < e1xpos + estretch1.w && stretch.x + stretch.w > e1xpos){
                                    e1alive = false;
                                }
                                if(stretch.y + stretch.h + 10 > e2ypos && stretch.y + stretch.h + 10 < e2ypos + estretch2.h && stretch.x < e2xpos + estretch2.w && stretch.x + stretch.w > e2xpos){
                                    e2alive = false;
                                }
                                if(stretch.y + stretch.h + 10 > bypos && stretch.y + stretch.h + 10 < bypos + bstretch.h && stretch.x < bxpos + bstretch.w && stretch.x + stretch.w > bxpos){
                                    balive = false;
                                }

                            }else if(frame == 2 || frame == 3){ // attack up
                                prev = frame;
                                frame = 9;
                                if(stretch.y - 10 < e0ypos + estretch0.h && stretch.y - 10 > e0ypos && stretch.x < e0xpos + estretch0.w && stretch.x + stretch.w > e0xpos){
                                    e0alive = false;
                                }
                                if(stretch.y - 10 < e1ypos + estretch1.h && stretch.y - 10 > e1ypos && stretch.x < e1xpos + estretch1.w && stretch.x + stretch.w > e1xpos){
                                    e1alive = false;
                                } 
                                if(stretch.y - 10 < e2ypos + estretch2.h && stretch.y - 10 > e2ypos && stretch.x < e2xpos + estretch2.w && stretch.x + stretch.w > e2xpos){
                                    e2alive = false;
                                } 
                                if(stretch.y - 10 < bypos + bstretch.h && stretch.y - 10 > bypos && stretch.x < bxpos + bstretch.w && stretch.x + stretch.w > bxpos){
                                    balive = false;
                                } 

                            }else if(frame == 4 || frame == 5){ // attack left
                                prev = frame;
                                frame = 10;
                                if(stretch.x - 10 < e0xpos + estretch0.w && stretch.x - 10 > e0xpos && stretch.y < e0ypos + estretch0.h && stretch.y + stretch.h > e0ypos){
                                    e0alive = false;
                                }
                                if(stretch.x - 10 < e1xpos + estretch1.w && stretch.x - 10 > e1xpos && stretch.y < e1ypos + estretch1.h && stretch.y + stretch.h > e1ypos){
                                    e1alive = false;
                                }
                                if(stretch.x - 10 < e2xpos + estretch2.w && stretch.x - 10 > e2xpos && stretch.y < e2ypos + estretch2.h && stretch.y + stretch.h > e2ypos){
                                    e2alive = false;
                                }
                                if(stretch.x - 10 < bxpos + bstretch.w && stretch.x - 10 > bxpos && stretch.y < bypos + bstretch.h && stretch.y + stretch.h > bypos){
                                    balive = false;
                                }

                            }else if(frame == 6 || frame == 7){ // attack right
                                prev = frame;
                                frame = 11;
                                if(stretch.x + stretch.w + 10 > e0xpos && stretch.x + stretch.w + 10 < e0xpos + estretch0.w && stretch.y < e0ypos + estretch0.h && stretch.y + stretch.h > e0ypos){
                                    e0alive = false;
                                }
                                if(stretch.x + stretch.w + 10 > e1xpos && stretch.x + stretch.w + 10 < e1xpos + estretch1.w && stretch.y < e1ypos + estretch1.h && stretch.y + stretch.h > e1ypos){
                                    e1alive = false;
                                }
                                if(stretch.x + stretch.w + 10 > e2xpos && stretch.x + stretch.w + 10 < e2xpos + estretch2.w && stretch.y < e2ypos + estretch2.h && stretch.y + stretch.h > e2ypos){
                                    e2alive = false;
                                }
                                if(stretch.x + stretch.w + 10 > bxpos && stretch.x + stretch.w + 10 < bxpos + bstretch.w && stretch.y < bypos + bstretch.h && stretch.y + stretch.h > bypos){
                                    balive = false;
                                }
                ...
                checkBounds(stretch, fullScreen, lxpos, lypos, lprevxpos, lprevypos, num, true, balive);
                ...
                if(prevnum != num){
                    switch(num){
                        ...
                        case 3:
                            ...
                            e2alive = true;
                            balive = false;
                            e0xpos = SCREEN_WIDTH / 2 - 20;
                            e0ypos = SCREEN_HEIGHT / 2 - 20;
                            e1xpos = 3*SCREEN_HEIGHT / 4;
                            e1ypos = 3*SCREEN_HEIGHT/ 4;
                            e2xpos = SCREEN_WIDTH / 4;
                            e2ypos = 3*SCREEN_HEIGHT / 4;
                            break;
                        case 1:
                            e0alive = true;
                            e1alive = false;
                        ...
                        case 4:
                            ...
                            e1alive = false;
                            e2alive = false;
                            bxpos = SCREEN_WIDTH / 2 - 30;
                            bypos = SCREEN_HEIGHT / 2 - 30;
                            break;
                        default:
                            balive = false;
                            e0alive = false;
                ...
                checkBounds(estretch0, fullScreen, e0xpos, e0ypos, e0prevxpos, e0prevypos, num, false, balive);
                checkBounds(estretch1, fullScreen, e1xpos, e1ypos, e1prevxpos, e1prevypos, num, false, balive);
                checkBounds(estretch2, fullScreen, e2xpos, e2ypos, e2prevxpos, e2prevypos, num, false, balive);
                checkBounds(bstretch, fullScreen, bxpos, bypos, bprevxpos, bprevypos, num, false, balive);
...
//	          gTreasureTexture.render(heartx, hearty, &treasure);
                ...
                if(e0alive){
                    SDL_RenderCopy(gRenderer, gEnemyTexture.texture(), &enemies[e0frame], &estretch0);
                }
                if(e1alive){
                    SDL_RenderCopy(gRenderer, gEnemyTexture.texture(), &enemies[e1frame], &estretch1);
                }
                if(e2alive){
                    SDL_RenderCopy(gRenderer, gEnemyTexture.texture(), &enemies[e2frame], &estretch2);
                }
